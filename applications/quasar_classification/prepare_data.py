...
def preprocess_data(input_filename='s82_pointsource_catalog.hdf',
  ...
  features = ['mjd_std',
              'psfMag_u_std', 'psfMag_g_std', 'psfMag_r_std', 'psfMag_i_std',
              'psfMag_z_std',
              'psfmagerr_u', 'psfmagerr_g', 'psfmagerr_r', 'psfmagerr_i',
              'psfmagerr_z', 'distance']

  dataset = Table.from_pandas(
    pd.read_hdf('s82_pointsource_catalog.hdf', '/lightcurve'))
  coadd_data = Table.from_pandas(
    pd.read_hdf('s82_pointsource_catalog.hdf', '/coadd'))
  ...
  for b in ['u', 'g', 'r', 'i', 'z']:
    dataset['psfMag_%s' % b] = dataset['psfMag_%s' % b] - dataset[
      'extinction_%s' % b]
    dataset['coadd_psfMag_%s' % b] = dataset['coadd_psfMag_%s' % b] - dataset[
      'coadd_extinction_%s' % b]
  ...
  dataset['distance'] = RobustScaler().fit_transform(
    dataset['distance'].reshape(-1, 1)).squeeze()

  # Sort the table by observation and time
  dataset.sort(['coadd_objid', 'mjd_std'])

  # count the number of observations for each entries
  unique_ids, idices, counts = np.unique(dataset['coadd_objid'],
                                         return_inverse=True,
                                         return_counts=True)
  ...
  for i in range(len(counts)):
    ...
    ts[i, :len(t), :] = np.array(t).view(np.float64).reshape(
      (-1, len(features))).astype(np.float32)
...
if __name__ == "__main__":
  parser = argparse.ArgumentParser(
    description='Creates a quasar light curve dataset.')
  ...
  for i in range(n_batch):
    ...
    casjobs.uploadPandasDataFrameToTable(
      ref_cat[i * BATCH_SIZE:(i + 1) * BATCH_SIZE], 'my_cat', context='MyDB')
