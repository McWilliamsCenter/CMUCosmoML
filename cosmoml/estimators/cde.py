...
def _mdn_model_fn(features, labels, hidden_units, n_mixture, diag,
  ...
  out_mu = slim.fully_connected(net, label_dimension * n_mixture,
                                activation_fn=None)
  out_mu = tf.reshape(out_mu, (-1, n_mixture, label_dimension))

  out_sigma = slim.fully_connected(net, size_sigma * n_mixture,
                                   activation_fn=None)
  ...
  if mode == tf.estimator.ModeKeys.PREDICT:
    ...
    return tf.estimator.EstimatorSpec(mode=mode, predictions=predictions,
                                      export_outputs={
                                        'pdf': tf.estimator.export.PredictOutput(
                                          predictions),
                                        'samples': tf.estimator.export.PredictOutput(
                                          samples),
  ...
  def __init__(self,
    ...
    def _model_fn(features, labels, mode):
      return _mdn_model_fn(features, labels, hidden_units, n_mixture, diagonal,
                           feature_columns, label_columns, optimizer,
                           activation_fn, normalizer_fn, dropout, mode)
